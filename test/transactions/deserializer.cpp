...
TEST(transactions, deserialize_multi_signature_registration)
{
    ...
    ASSERT_EQ(actual.header, 0xFF);
    ASSERT_EQ(actual.version, 1);
    ASSERT_EQ(actual.network, 23);
    ASSERT_EQ(actual.type, Ark::Crypto::Enums::Types::MULTI_SIGNATURE_REGISTRATION);
    ASSERT_EQ(actual.timestamp, 10112114);
    ASSERT_EQ(actual.senderPublicKey, "036928c98ee53a1f52ed01dd87db10ffe1980eb47cd7c0a7d688321f47b5d7d760");
    ...
    ASSERT_EQ(actual.signatures, signatures);
